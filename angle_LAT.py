...
#     print(c_point)
    errorcode1 = 0
    center_x = c_point[int(len(c_point)/2)]
    center_x2 = c_point[int(len(c_point)/2-60)]
    for i in range(int(len(c_point)*0.3),int(len(c_point)*0.7)):
        if abs(c_point[i+1][0]-c_point[i][0])>5:
#             print(abs(c_point[i+1][0]-c_point[i][0]))
            errorcode1 = 1
            center_x = c_point[int(len(c_point)/3)]
            center_x2 = c_point[int(len(c_point)/3-60)]
#         else:
            
#     center_x2 = c_point[int(len(c_point)/3-int(len(c_point)*0.1))]

    
    return x_list, y_list, center_x, center_x2, errorcode1

def find_rotate_center_point(contour, errorcode1):
    c_point = []
    a=contour[0].tolist()
    x_list, y_list =[], []
